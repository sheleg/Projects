...
public class Index implements Serializable, Closeable {
    ...
    IndexOne2One          name;
    IndexOne2N            trainer;
    IndexOne2One          startTime;
	
    public void test(ComplexMember member) throws KeyNotUniqueException {
        assert(member != null );
        if ( name.contains(member.name)) {
            throw new KeyNotUniqueException(member.name);
        }
        if ( startTime.contains(member.name )) {
            throw new KeyNotUniqueException(member.name );
        }		
    }
	
    public void put(ComplexMember member, long value ) throws KeyNotUniqueException {
        test(member );
        name.put(member.name, value );
        trainer.put(member.trainer, value);
        startTime.put(member.name, value);		
    }
	
    public Index()  {
        name   = new IndexOne2One();
        trainer = new IndexOne2N();
        startTime 	= new IndexOne2One();
    }
